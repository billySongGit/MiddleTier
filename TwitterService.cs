...
namespace Sabio.Web.Services
{
    public class TwitterService : BaseService, ITwitterService
    {
        ...
        public async Task<List<TwitterStatus>> GetTweetsByHandle(string handle, int pgIndex = 0, int pgSize = 10)
        {
            ...
            if (setting != null)
            {
                var key = "twitter_" + setting.Value + "_pg_" + pgIndex;
                ...
                if (!_cacheService.Contains(key))
                {
                    ...
                    _cacheService.Add(key, tweets, new DateTimeOffset(DateTime.Now.AddHours(1)));
                }
                else
                {
        ...
        public async Task<List<TwitterStatus>> GetTweets(string maxIdStr)
        {
            /* Gets the MaxID from the database. */
            ulong storedMaxId = 0;
            ...
            var tweets =
                ...
                where
                    tweet.Type == StatusType.User &&
                    tweet.ScreenName == "lashgirlstudio" &&
                    tweet.Count == 10 &&
                    tweet.MaxID == storedMaxId - 1 &&
        ...
        public async Task<List<TwitterStatus>> GetTweets()
        {
            DeleteMaxId(); // Empty the TwitterMaxId table
